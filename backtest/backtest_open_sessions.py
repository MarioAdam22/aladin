...
# ── CONFIG ────────────────────────────────────────────────────────────────────
MODEL_PATH    = DIR / "mario_bot_open.json"
FEATURES_PATH = DIR / "mario_bot_open_features.json"
...
# Entry windows — Eastern Time (UTC-5 winter / UTC-4 summer)
# LON: 2:00-5:00 AM ET  = 07:00-10:00 UTC
# NY:  8:00-11:00 AM ET = 13:00-16:00 UTC
LON_ENTRY_START, LON_ENTRY_END = 7.0,  10.0
NY_ENTRY_START,  NY_ENTRY_END  = 13.0, 16.0
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = {}
    out['in_london_or']    = ((td>=7.0)&(td<7.5)).astype(np.int8)
    out['in_london_kz']    = ((td>=7.5)&(td<10.0)).astype(np.int8)
    out['in_london_close'] = ((td>=10.0)&(td<11.0)).astype(np.int8)
    out['in_pre_ny']       = ((td>=13.0)&(td<13.5)).astype(np.int8)
    out['in_ny_or']        = ((td>=13.5)&(td<14.5)).astype(np.int8)
    out['in_ny_kz_core']   = ((td>=14.5)&(td<15.0)).astype(np.int8)
    out['in_ny_macro_1']   = ((td>=15.0)&(td<15.5)).astype(np.int8)
    out['in_ny_macro_2']   = ((td>=15.5)&(td<16.0)).astype(np.int8)
    out['in_any_macro']    = ((out['in_ny_macro_1']==1)|(out['in_ny_macro_2']==1)).astype(np.int8)
    out['mins_since_lon_open'] = np.clip((td-7.0)*60,0,180).astype(np.float32)
    out['mins_since_ny_open']  = np.clip((td-14.5)*60,0,120).astype(np.float32)
...
def main():
    ...
    df_t.to_csv(DIR/"backtest_open_sessions_trades.csv", index=False)
    print(f"\n✅ Trades salvate: backtest_open_sessions_trades.csv")
