"""
Backtest mario_bot_v3.json + simulare prop firm Lucid Trading
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
MODEL_PATH    = DIR / "mario_bot_v3.json"
FEATURES_PATH = DIR / "mario_bot_v3_features.json"
...
# ── FEATURE COMPUTATION (identic cu train_mario_bot_v3.py) ───────────────────
...
def main():
    print("═"*65)
    print("  BACKTEST + PROP FIRM — mario_bot_v3.json  (2023-2025)")
    print("═"*65)
    ...
    # ── ICT PRE-FILTER (vectorizat) ───────────────────────────────────────────
    # Aplicăm aceleași condiții ca în target generation v3 ÎNAINTE de model.
    # Modelul antrenat pe bare ICT se comportă imprevizibil pe bare fără setup.
    feat_cl  = feat_df['close'].values.astype(float)
    feat_hi  = feat_df['high'].values.astype(float)
    feat_lo  = feat_df['low'].values.astype(float)
    feat_atr = np.where(feat_df['atr_14'].values > 0, feat_df['atr_14'].values, 9.0).astype(float)
    feat_op  = df_ohlc['open'].values.astype(float)   # df_ohlc aliniat cu feat_df

    oc_pdh = pd.Series(df_ohlc['p_hi'].values).ffill().values.astype(float)
    oc_pdl = pd.Series(df_ohlc['p_lo'].values).ffill().values.astype(float)
    oc_ahi = pd.Series(df_ohlc['asia_hi'].values).ffill().values.astype(float)
    oc_alo = pd.Series(df_ohlc['asia_lo'].values).ffill().values.astype(float)
    oc_lhi = pd.Series(df_ohlc['lon_hi'].values).ffill().values.astype(float)
    oc_llo = pd.Series(df_ohlc['lon_lo'].values).ffill().values.astype(float)

    # key levels vectorizate
    key_hi = np.fmax(np.fmax(oc_ahi, oc_lhi), oc_pdh)  # nanmax cu fill -inf
    key_lo = np.fmin(np.fmin(oc_alo, oc_llo), oc_pdl)  # nanmin cu fill +inf

    bar_range = np.maximum(feat_hi - feat_lo, 0.01)
    close_str  = (feat_cl - feat_lo) / bar_range        # 0..1, aproape 1 = bullish
    top_close  = (feat_hi - feat_cl) / bar_range        # 0..1, aproape 1 = bearish

    tol = 0.05 * feat_atr

    # LONG_REV: bara atinge/depășește key_low + bullish rejection
    long_rev  = (feat_lo <= key_lo + tol) & (feat_cl > feat_op) & (close_str > 0.60)
    # SHORT_REV: bara atinge/depășește key_high + bearish rejection
    short_rev = (feat_hi >= key_hi - tol) & (feat_cl < feat_op) & (top_close > 0.60)
    # LONG_BREAK: rupe deasupra key_high, inchide puternic, bara de impuls
    long_brk  = (feat_hi > key_hi) & (feat_cl >= key_hi - tol) & (close_str > 0.55) & (bar_range > 0.8*feat_atr)
    # SHORT_BREAK: rupe sub key_low, inchide slab, bara de impuls
    short_brk = (feat_lo < key_lo) & (feat_cl <= key_lo + tol) & (top_close > 0.55) & (bar_range > 0.8*feat_atr)

    ict_match = long_rev | short_rev | long_brk | short_brk

    has_signal = np.isin(pred, SIGNAL_CLASSES) & (conf >= CONF_THRESHOLD) & ict_match
    entry_mask = in_entry & has_signal

    entry_indices = np.where(entry_mask)[0]
    print(f"✓ Bare cu setup ICT: {ict_match.sum():,} / {len(feat_df):,}")
    print(f"✓ Semnale după ICT + model (conf>={CONF_THRESHOLD}): {len(entry_indices):,}")
    ...
    df_t.to_csv(DIR/"backtest_mario_bot_v3_trades.csv", index=False)
    print(f"\n✅ Trades salvate: backtest_mario_bot_v3_trades.csv")
