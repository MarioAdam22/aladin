"""
ALADIN — LON Turtle Soup Quality Gate v1
══════════════════════════════════════════════════════════════════════════
Turtle Soup = false breakout reversal după sweep Asia Hi/Lo
Față de v6 (toate trade-urile LON), ts_lon_v1 adaugă:
  ✅ Filter: NUMAI SHORT_REV + LONG_REV (true TS reversal trades)
  ✅ ts_sweep_depth_pts / _atr       — cât de adânc a intrat în lichiditate
  ✅ ts_close_inside                 — bar-ul a închis ÎNAPOI în range (clean rejection)
  ✅ ts_rejection_str                — puterea revenirii (close vs nivel sweepat)
  ✅ ts_wick_pct                     — wick dincolo de nivel / range total bar
  ✅ ts_body_pct                     — raport corp/range (pin bar quality)
  ✅ ts_close_quality                — close position în bar (SHORT: low close = bun)
  ✅ ts_wick_dom                     — dominanța wick-ului direcțional
  ✅ ts_asia_rng_atr                 — lărgimea sesiunii Asia (lichiditate disponibilă)
  ✅ ts_sweep_pct_asia               — sweep depth ca % din range Asia
  ✅ ts_opp_dist_atr                 — distanța la lichiditatea opusă (reward potential)
  ✅ ts_rr_impl                      — RR implicit (reward / sweep depth)
  ✅ ts_sharp                        — flag: sweep adânc + close înapoi curat
  ✅ ts_entry_prox                   — cât de aproape e entry-ul de nivelul sweepat
  ✅ ts_near_wk_extreme              — sweep-ul atinge un extrem săptămânal
  ✅ ts_htf_anti                     — H4 bias opus direcției sweep (confirmă reversalul)

Toate celelalte features din v6 (147) sunt păstrate → total ~163 features
"""
...
print("=" * 74)
print("train_quality_ts_lon_v1.py — LON Turtle Soup Quality Gate v1")
print("=" * 74)

# ════════════════════════════════════════════════════════════════════════
# ICT on TF (identical to v6)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# STEP 1: Load CSV — filter LON + REVERSAL only
# ════════════════════════════════════════════════════════════════════════
print("\n▶  [1/7] Încărcare backtest CSV — LON REVERSAL only ...")
df_csv = pd.read_csv(CSV_PATH)
df_all = df_csv[df_csv['session'] == 'LON'].copy()

# TS FILTER: only reversal trades (SHORT_REV = sweep hi → short, LONG_REV = sweep lo → long)
ts_regimes = ['SHORT_REV', 'LONG_REV']
df_lon = df_all[df_all['regime'].isin(ts_regimes)].copy()

print(f"   LON total:    {len(df_all):,}")
print(f"   LON reversal: {len(df_lon):,} ({len(df_lon)/len(df_all)*100:.1f}%)")
print(f"   Excluded breakouts: {len(df_all)-len(df_lon):,}")
...
print(f"   Trail rate: {df_lon['trail'].mean()*100:.2f}%")
...
print(f"   DB rows joined: {len(db):,} / {len(df_lon):,}")
...
# Rolling 5-session WR (from ALL LON reversal trades)
df_lon_sorted = df_lon.sort_values('ts').copy()
if 'trail' not in df_lon_sorted.columns:
    df_lon_sorted['trail'] = (df_lon_sorted['exit_reason']=='TRAIL').astype(int)
...
# ════════════════════════════════════════════════════════════════════════
# STEP 4: Feature Engineering (v6 + TS features)
# ════════════════════════════════════════════════════════════════════════
print("\n▶  [4/7] Feature engineering (v6 + TS features) ...")
...
# ── SWEEP QUALITY (v6)
sweep_level_v6=np.where(trade_dir==1,asia_lo,asia_hi)
dist_to_pdl=np.abs(sweep_level_v6-p_lo_arr); dist_to_pdh=np.abs(sweep_level_v6-p_hi_arr)
feat['equal_level_score']  = clip(1.0-safe_norm(np.minimum(dist_to_pdl,dist_to_pdh),atr))
sweep_depth_v6=np.where(trade_dir==1,sweep_level_v6-cl,cl-sweep_level_v6)
feat['sweep_depth_atr']    = safe_norm(sweep_depth_v6,atr)
feat['deep_sweep']         = (sweep_depth_v6>atr*0.4).astype(float)
feat['shallow_sweep']      = (sweep_depth_v6<atr*0.1).astype(float)
...
# ════════════════════════════════════════════════════════════════════════
# SECTION TS: TURTLE SOUP SPECIFIC FEATURES (noi în v1)
# ════════════════════════════════════════════════════════════════════════
# dir_short=1 → SHORT_REV: swept asia_hi (high above level, close back below)
# dir_short=0 → LONG_REV:  swept asia_lo (low below level, close back above)

dir_s = df['dir_short'].values.astype(float)

# Nivelul sweepat
swept_lvl = np.where(dir_s==1, asia_hi, asia_lo)

# 1. Adâncimea sweep-ului (cât de departe a mers wick-ul dincolo de nivel)
ts_depth_pts = np.where(dir_s==1,
    np.maximum(hi - swept_lvl, 0.0),    # SHORT: high above asia_hi
    np.maximum(swept_lvl - lo, 0.0))    # LONG: asia_lo above low
feat['ts_sweep_depth_pts']  = ts_depth_pts
feat['ts_sweep_depth_atr']  = safe_norm(ts_depth_pts, atr)

# 2. Close ÎNAPOI în range (same bar rejection)
ts_close_inside = np.where(dir_s==1,
    (cl < swept_lvl).astype(float),     # SHORT: close below asia_hi
    (cl > swept_lvl).astype(float))     # LONG: close above asia_lo
feat['ts_close_inside']     = ts_close_inside

# 3. Puterea reversal-ului (cât de adânc e close-ul înapoi în range)
ts_rejection_str = np.where(dir_s==1,
    safe_norm(swept_lvl - cl, atr),     # SHORT: asia_hi - close
    safe_norm(cl - swept_lvl, atr))     # LONG: close - asia_lo
feat['ts_rejection_str']    = ts_rejection_str

# 4. Wick dincolo de nivel ca % din range total bar (pin bar quality)
bar_rng = np.maximum(hi - lo, 0.1)
feat['ts_wick_pct']         = ts_depth_pts / bar_rng

# 5. Corp mic = pin bar bun (small body / range)
body_sz = np.abs(cl - op)
feat['ts_body_pct']         = body_sz / bar_rng

# 6. Close position în bar (SHORT: vrem close spre low; LONG: spre high)
...
feat['ts_wick_dom']         = np.where(dir_s==1,
    safe_norm(upper_wick, np.maximum(lower_wick, 0.1)),  # SHORT: upper >> lower
    safe_norm(lower_wick, np.maximum(upper_wick, 0.1)))  # LONG: lower >> upper

# 8. Asia range ca context (cât de lată a fost sesiunea Asia)
asia_rng = np.maximum(asia_hi - asia_lo, 0.1)
feat['ts_asia_rng_atr']     = safe_norm(asia_rng, atr)

# 9. Sweep ca % din range Asia (sweep adânc vs superficial)
feat['ts_sweep_pct_asia']   = ts_depth_pts / asia_rng

# 10. Distanța la lichiditatea opusă (reward potential, ATR-normalized)
feat['ts_opp_dist_atr']     = np.where(dir_s==1,
    safe_norm(swept_lvl - asia_lo, atr),   # SHORT: target = asia_lo
    safe_norm(asia_hi - swept_lvl, atr))   # LONG: target = asia_hi

# 11. RR implicit (distanța la lichiditate opusă / adâncimea sweep-ului)
feat['ts_rr_impl']          = safe_norm(
    np.where(dir_s==1, swept_lvl-asia_lo, asia_hi-swept_lvl),
    np.maximum(ts_depth_pts, 0.1))

# 12. Sharp rejection: sweep decent + close curat (semnal puternic TS)
feat['ts_sharp']            = ((ts_depth_pts > 0.3*atr) & (ts_close_inside==1.0)).astype(float)

# 13. Entry proximity la nivel (cât de aproape suntem de nivel = timing bun)
feat['ts_entry_prox']       = np.where(dir_s==1,
    safe_norm(swept_lvl - np.minimum(cl, op), atr),
    safe_norm(np.maximum(cl, op) - swept_lvl, atr))

# 14. Sweep atinge un extrem săptămânal (mai mult interes instituțional)
feat['ts_near_wk_extreme']  = np.where(dir_s==1,
    (np.abs(swept_lvl - wk_hi_sf) < atr*0.5).astype(float),   # SHORT: sweep near week high
    (np.abs(swept_lvl - wk_lo_sf) < atr*0.5).astype(float))   # LONG: sweep near week low

# 15. H4 bias OPUS direcției sweep → confirmă reversalul
h4_mid = (h4h + h4l) / 2
feat['ts_htf_anti']         = np.where(dir_s==1,
    (h4_mid < cl).astype(float),    # SHORT TS: H4 midpoint below close = bearish H4
    (h4_mid > cl).astype(float))    # LONG TS: H4 midpoint above close = bullish H4

# 16. Combinat TS quality: close inside + wick dominant + sharp
...
print(f"   V6 features:  ~147")
print(f"   TS features:  16")
print(f"   Regime features: 9")
...
SAVE_DIR = DIR.parent
MODEL_NAME = 'mario_quality_ts_lon_v1'
...
for regime_name in ACTIVE_REGIMES:
    ...
    if mdl is not None:
        ...
        if regime_name != 'ALL':
            _rpkl = DIR.parent / f"mario_quality_ts_lon_v1_{regime_name}_calibrated.pkl"
...
# Save
model_path=DIR.parent/"mario_quality_ts_lon_v1.json"
meta_path =DIR.parent/"mario_quality_ts_lon_v1_features.json"
cal_path  =DIR.parent/"mario_quality_ts_lon_v1_calibrated.pkl"
...
meta={
    "version":"ts_lon_v1","session":"LON","filter":"SHORT_REV+LONG_REV only",
